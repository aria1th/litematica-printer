...
public class Printer {
                                ...
                                if (cBlock.getName().equals(sBlock.getName())) {
                                    ...
                                    if (facingSchematic == facingClient) {
                                        ...
                                        if (sBlock instanceof RepeaterBlock && !LitematicaMixinMod.PRINTER_ACCURATE_BLOCK_PLACEMENT.getBooleanValue()) {
                                             ...
                                             */
                                            if (stateClient.get(LeverBlock.FACE) == BlockFace.CEILING) {
                                                side = Direction.DOWN;
                                            } else if (stateClient.get(LeverBlock.FACE) == BlockFace.FLOOR) {
                                                side = Direction.UP;
                                            } else {
                    ...
                    if (canPickBlock(mc, stateSchematic, pos)) {
                        ...
                        if (blockSchematic instanceof WallMountedBlock || blockSchematic instanceof TorchBlock || blockSchematic instanceof WallSkullBlock
                            ...
                            if (blockSchematic instanceof ButtonBlock || blockSchematic instanceof LeverBlock) {
                                BlockFace wallMountLocation = stateSchematic.get(WallMountedBlock.FACE);
                                if (wallMountLocation == BlockFace.FLOOR) {
                                    npos = pos.down();
                                } else if (wallMountLocation == BlockFace.CEILING) {
                                    npos = pos.up();
                                } else {
    ...
    private static boolean ObserverCantAvoid(MinecraftClient mc, World world, Direction facingSchematic, BlockPos pos) {
        ...
        if (facingSchematic.equals(Direction.UP)) {
            return offsetBlock instanceof WallBlock || offsetBlock instanceof ComparatorBlock || offsetBlock instanceof DoorBlock ||
                    ...
                    ((offsetBlock instanceof WallMountedBlock) && OffsetStateSchematic.get(WallMountedBlock.FACE) == BlockFace.FLOOR);
        } else if (facingSchematic.equals(Direction.DOWN)) {
            return offsetBlock instanceof WallBlock || offsetBlock instanceof WallMountedBlock && OffsetStateSchematic.get(WallMountedBlock.FACE) == BlockFace.CEILING;
        } else {
            return offsetBlock instanceof WallBlock || offsetBlock instanceof PaneBlock || offsetBlock instanceof FenceBlock || OffsetStateSchematic.isOf(Blocks.IRON_BARS) || offsetBlock instanceof WallMountedBlock &&
                    OffsetStateSchematic.get(WallMountedBlock.FACE) == BlockFace.WALL && OffsetStateSchematic.get(WallMountedBlock.FACING) == facingSchematic || hasDustOrAscendingRails(world, facingSchematic, pos);
        }
    }
    ...
    private static Set<BlockPos> ObserverCantAvoidPos(MinecraftClient mc, World world, BlockPos pos) {
        ...
        if (block instanceof ComparatorBlock || block instanceof RepeaterBlock || block instanceof FallingBlock ||
                ...
                ((block instanceof WallMountedBlock) && targetState.get(WallMountedBlock.FACE) == BlockFace.FLOOR)) {
            //check downward
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (offsetStateSchematic.isOf(Blocks.OBSERVER) && offsetStateSchematic.get(ObserverBlock.FACING) == direction.getOpposite()) {
                if (block instanceof WallBlock) {
                    ...
                    if (targetState.get(WallMountedBlock.FACE) == BlockFace.CEILING && direction == Direction.UP ||
                            targetState.get(WallMountedBlock.FACE) == BlockFace.FLOOR && direction == Direction.DOWN ||
                            targetState.get(WallMountedBlock.FACE) == BlockFace.WALL && targetState.get(WallMountedBlock.FACING) == direction.getOpposite()
                    ) {
                        relatedPos.add(pos.offset(direction, 2));
                        relatedPos.addAll(getNeighborsExcept(pos, direction));
                        if (world.getBlockState(pos.offset(direction, 2).down()).getBlock() instanceof PistonBlock) {
    ...
    private static void placeGrindStone(BlockState state, MinecraftClient client, BlockPos pos) {
        ...
        BlockFace location = state.get(GrindstoneBlock.FACE);
        BlockPos clickPos;
        Vec3d hitVec;
        ...
        if (facing != null && facedata != null) {
            ...
            switch (facedata.type) {
                ...
                case 2: // Wall mountable, such as a lever, only use player direction if not on wall.
                    return stateSchematic.get(WallMountedBlock.FACE) == BlockFace.WALL
                            || (facing == horizontalFacing && stateSchematic.get(WallMountedBlock.FACE) == BlockFace.CEILING ? (primaryFacing == Direction.UP && horizontalFacing == stateSchematic.get(WallMountedBlock.FACING)) : (primaryFacing == Direction.DOWN && horizontalFacing == stateSchematic.get(WallMountedBlock.FACING)));
    ...
    public static Vec3d applyHitVec(BlockPos pos, BlockState state, Direction side) {
        ...
        if (!(block instanceof GrindstoneBlock) && block instanceof WallMountedBlock || block instanceof TorchBlock || block instanceof WallSkullBlock
            ...
            if (block instanceof DeadCoralFanBlock && !(block instanceof DeadCoralWallFanBlock)) {
                ...
                side = Direction.UP;
                clickPos = Vec3d.ofCenter(pos.down()).add(Vec3d.of(side.getVector()).multiply(0.5));
            } else if (side == null || state.contains(WallMountedBlock.FACE) && state.get(WallMountedBlock.FACE) != BlockFace.WALL) {
                if (state.contains(WallMountedBlock.FACE) && state.get(WallMountedBlock.FACE) == BlockFace.CEILING) {
                    side = Direction.DOWN;
                } else {
                    side = Direction.UP;
                }
    ...
    private static void updateSignText(MinecraftClient mc, World schematicWorld, BlockPos pos) {
        ...
        if (entity instanceof SignBlockEntity signBlockEntity && clientEntity instanceof SignBlockEntity clientSignEntity) {
            if (clientSignEntity.getText(false).getMessage(0, false).getContent() != PlainTextContent.EMPTY ||
                    clientSignEntity.getText(false).getMessage(1, false).getContent() != PlainTextContent.EMPTY ||
                    clientSignEntity.getText(false).getMessage(2, false).getContent() != PlainTextContent.EMPTY ||
                    clientSignEntity.getText(false).getMessage(3, false).getContent() != PlainTextContent.EMPTY ) {
    ...
    static Direction applyPlacementFacing(BlockState stateSchematic, Direction side, BlockState stateClient) {
        ...
        if (blockSchematic instanceof SlabBlock) {
            ...
            BlockFace location = stateSchematic.get(WallMountedBlock.FACE);
            if (location == BlockFace.FLOOR) {
                return Direction.UP;
            } else if (location == BlockFace.CEILING) {
                return Direction.DOWN;
            } else {
